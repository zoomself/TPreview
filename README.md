...
1. **增量下载**：按 `config.yaml` 的日期区间拉取日 K；本地已有 CSV 时比对最后一根 K 线的日期，从**下一交易日**继续拉取，跳过已完成股票。每次同步结束会按 `data_retention_days`（默认约两个月）**丢弃过旧行**，减轻仓库体积。
...
4. **分析报告**：列出「截至本地最新一根 K」时，**连续下跌天数 ≥ 配置阈值**（默认 5）的股票；**排除** `stocks.csv` 中标记为 ST 的证券。页面将**代码并入名称**展示；字段含最新收盘、连跌天数、连跌总跌幅、近 N 个交易日总跌幅；**相同连跌天数**共用同一浅色背景分组。按连跌天数降序，同天数下按连跌总跌幅从大到小（跌幅更深优先）。每条卡片可 **展开** 查看 streak 内逐日涨跌幅。
...
| `data_retention_days` | 日 K 本地保留窗口：以**该 CSV 最后一根 K 的日期**为锚，删除早于此日期往前推 N 个**自然日**的行（默认 `62` 约两个月）；`0` 表示不裁剪。每次同步结束会对股票池内各 `data/daily/*.csv` 执行一次。不宜设得过短，应明显大于 `lookback_trading_days` 所需的历史跨度，否则报告里的近 N 日跌幅等常为 `—` |
...
本项目仅供学习与技术研究使用。证券行情来自第三方（Baostock），**不保证**实时性、完整性与准确性。**不构成投资建议**。使用与传播数据时请遵守 Baostock 及数据源的相关条款。
